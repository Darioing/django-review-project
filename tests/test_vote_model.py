...
def test_user_id_filled_correctly(user, review):
    ...
    vote = Votes.objects.create(
        user_id=user, content_type=content_type, object_id=review.id, vote_type=1)
...
def test_user_id_empty(review):
    ...
    with pytest.raises(IntegrityError):
        Votes.objects.create(
            user_id=None, content_type=content_type, object_id=review.id, vote_type=1)
...
def test_content_type_questions(user, question):
    ...
    vote = Votes.objects.create(
        user_id=user, content_type=content_type, object_id=question.id, vote_type=1)
    assert vote.content_type == content_type
...
def test_content_type_comments(user, comment):
    ...
    vote = Votes.objects.create(
        user_id=user, content_type=content_type, object_id=comment.id, vote_type=1)
    assert vote.content_type == content_type
...
def test_vote_type_invalid(user, review):
    ...
    with pytest.raises(ValidationError):
        vote = Votes(user_id=user, content_type=content_type,
                     object_id=review.id, vote_type=2)
        vote.full_clean()
...
def test_str_method(user, review):
    ...
    vote = Votes.objects.create(
        user_id=user, content_type=content_type, object_id=review.id, vote_type=1)
    expected_str = f"{vote.user_id} оставил {
        vote.vote_type} к {vote.content_object}"
