...
def test_place_id_empty(user):
    ...
    with pytest.raises(IntegrityError):
        Questions.objects.create(
            place_id=None, user_id=user, text="Как работают по выходным?")
...
def test_user_id_empty(place):
    ...
    with pytest.raises(IntegrityError):
        Questions.objects.create(
            place_id=place, user_id=None, text="Как работают по выходным?")
...
def test_str_method(user, place):
    ...
    question = Questions.objects.create(
        place_id=place, user_id=user, text="Как работают по выходным?")
    expected_str = f"{question.user_id} {
        question.place_id} {question.text[:30]}"
