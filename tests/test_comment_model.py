...
def user():
    ...
    return User.objects.create(
        first_name="test", second_name="user", email="test@user.ru", password="password"
    )
...
def review(user, place):
    ...
    return Reviews.objects.create(
        place_id=place, user_id=user, text="Отличное место!", price=5, service=5, interior=5
    )
...
def test_user_id_filled_correctly(user, review):
    ...
    comment = Comments.objects.create(
        user_id=user, content_type=content_type, object_id=review.id, text="Согласен с отзывом!"
    )
...
def test_user_id_empty(review):
    ...
    with pytest.raises(IntegrityError):
        Comments.objects.create(
            user_id=None, content_type=content_type, object_id=review.id, text="Согласен с отзывом!"
        )
...
def test_content_type_questions_filled_correctly(user, question):
    ...
    comment = Comments.objects.create(
        ...
        object_id=question.id, text="Спасибо за информацию!"
    )
    assert comment.content_type == content_type
...
def test_text_exceeds_max_length(user, review):
    ...
    with pytest.raises(ValidationError):
        comment = Comments(user_id=user, content_type=content_type,
                           object_id=review.id, text=long_text)
        comment.full_clean()
...
def test_text_empty(user, review):
    ...
    with pytest.raises(ValidationError):
        comment = Comments(
            user_id=user, content_type=content_type, object_id=review.id, text="")
        comment.full_clean()
...
def test_str_method(user, review):
    ...
    expected_str = f"{comment.user_id} {
        comment.text[:30]} {comment.content_object}"
